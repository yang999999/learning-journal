...
## 📌 重点项目阶段（W6-W12）新增项目相关内容

完成两个核心项目：

### 项目A：企业级 RAG 智能知识库
**重点学习**：
- [ ] RAG 全链路优化（切片/混合检索/Rerank/HyDE）
- [ ] 幻觉治理（引用来源/Faithfulness校验/用户反馈）
- [ ] 工程化（缓存/模型路由/流式/权限/可观测）
- [ ] 端到端测试和评估（RAGAS 指标）
**产出**：可运行的最小系统 + 量化评估报告 + 架构图

### 项目B：AI 运维 Copilot / Agent 故障诊断
**重点学习**：
- [ ] LangGraph 状态机 + Supervisor-Worker 多 Agent 架构
- [ ] MCP 协议理解与工具接入
- [ ] ReAct 循环 + 人机协作 + 安全管控
- [ ] 构建至少 1 个完整诊断流程 demo
**产出**：架构图 + 完整故事线 + 最小可运行 demo
